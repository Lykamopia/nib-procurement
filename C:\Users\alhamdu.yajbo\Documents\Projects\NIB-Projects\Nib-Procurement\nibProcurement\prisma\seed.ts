...
async function main() {
  ...
  // Seed Vendors and their corresponding Users
  for (const vendor of seedData.vendors) {
      const { kycDocuments, userId, ...vendorData } = vendor;
      const vendorUserSeed = seedData.users.find(u => u.id === userId);
      if (!vendorUserSeed) {
          console.warn(`Vendor user with id ${userId} not found for vendor ${vendor.name}. Skipping.`);
          continue;
      }
      ...
      const createdVendor = await prisma.vendor.create({
        data: {
          ...
          user: {
            create: {
              id: vendorUserSeed.id,
              name: vendorUserSeed.name,
              email: vendorUserSeed.email,
              password: vendorUserSeed.password,
              role: 'Vendor'
            }
          }
        },
  ...
  for (const requisition of seedData.requisitions) {
      ...
      // Assign transactionId after creation
      await prisma.purchaseRequisition.update({
        where: { id: createdRequisition.id },
        data: { transactionId: createdRequisition.id },
      });
  }
  console.log('Seeded requisitions and related items/questions/criteria.');
   ...
   for (const quote of seedData.quotations) {
       const { items, answers, scores, requisitionId, vendorId, ...quoteData } = quote;
        const requisitionForQuote = await prisma.purchaseRequisition.findUnique({ where: { id: requisitionId } });
        if (!requisitionForQuote) continue;

       const createdQuote = await prisma.quotation.create({
           data: {
               ...quoteData,
               transactionId: requisitionForQuote.transactionId,
   ...
   // Seed Purchase Orders
    for (const po of seedData.purchaseOrders) {
        ...
        const requisitionForPO = await prisma.purchaseRequisition.findUnique({ where: { id: po.requisitionId } });
        if (!requisitionForPO) continue;

        const createdPO = await prisma.purchaseOrder.create({
            data: {
                ...poData,
                transactionId: requisitionForPO.transactionId,
    ...
    for (const invoice of seedData.invoices) {
        ...
        const poForInvoice = await prisma.purchaseOrder.findUnique({ where: { id: invoice.purchaseOrderId } });
        if (!poForInvoice) continue;

        const createdInvoice = await prisma.invoice.create({
            data: {
                ...invoiceData,
                transactionId: poForInvoice.transactionId,
    ...
    for (const grn of seedData.goodsReceipts) {
        ...
        const poForGrn = await prisma.purchaseOrder.findUnique({ where: { id: grn.purchaseOrderId } });
        if (!poForGrn) continue;

        const createdGrn = await prisma.goodsReceiptNote.create({
            data: {
                ...grnData,
                transactionId: poForGrn.transactionId,
