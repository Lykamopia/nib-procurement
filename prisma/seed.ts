...
async function main() {
  ...
  const allRoles = [
      ...
      { name: 'ManagerProcurementDivision', description: 'Approves low-value awards.' },
      { name: 'DirectorSupplyChainandPropertyManagement', description: 'Approves mid-value awards.' },
      { name: 'VPResourcesandFacilities', description: 'Approves high-value awards.' },
  ...
  for (const user of seedData.users.filter(u => u.role !== 'Vendor')) {
    ...
    await prisma.user.create({
      data: {
          ...
          role: userData.role.replace(/ /g, ''), // Pass role as a string
  ...
  for (const vendor of seedData.vendors) {
      ...
      const createdUser = await prisma.user.create({
          data: {
              ...
              role: vendorUser.role.replace(/ /g, ''), // Pass role as a string
          }
      });
